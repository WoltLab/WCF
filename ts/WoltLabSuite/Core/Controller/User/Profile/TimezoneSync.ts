...
import { getStoragePrefix, getXsrfToken } from "../../../Core";
...
function getSessionStorageKey(): string {
  return `${getStoragePrefix()}tz-sync`;
}

function hasPreviouslyRequestedAnUpdate(): boolean {
  ...
  try {
    ...
    if (value !== null) {
      ...
      const OneDayAgo = Date.now() - 86_400;
