...
try {
    if (WCF::getLanguage() !== null) {
        ...
        $exceptionSubtitle = \str_replace('{$exceptionID}', $exceptionID,
            WCF::getLanguage()->get('wcf.global.exception.subtitle', true));
        $exceptionExplanation = \str_replace('{$logFile}', $logFile,
...
<div class="exceptionContainer">
	<div class="exceptionHeader">
		<div class="exceptionBoundary">
			...
			<p class="exceptionErrorCode"><?php echo \str_replace('{$exceptionID}', $exceptionID,
                                $exceptionSubtitle); ?></p>
		</div>
	</div>
    ...
    <?php if (!\defined('EXCEPTION_PRIVACY') || EXCEPTION_PRIVACY !== 'private') { ?>
	    <div class="exceptionBoundary">
		    ...
		    <ul class="exceptionSystemInformation">
			    <li class="exceptionSystemInformation1">
				    ...
				    <p class="exceptionFieldValue"><?php echo StringUtil::encodeHTML(\PHP_VERSION); ?></p>
			    </li>
			    <li class="exceptionSystemInformation3">
				    <p class="exceptionFieldTitle">WoltLab Suite Core<span
			    ...
			    <li class="exceptionSystemInformation5">
				    ...
				    <p class="exceptionFieldValue"><?php echo \round(\memory_get_peak_usage() / 1024 / 1024,
                                            3); ?>/<?php echo \round(FileUtil::getMemoryLimit() / 1024 / 1024, 3); ?>
					    MiB</p>
			    </li>
		...
		<div class="exceptionBoundary">
			...
			<ul class="exceptionErrorDetails">
				<li>
					<p class="exceptionFieldTitle">Error Type<span class="exceptionColon">:</span>
					</p>
					<p class="exceptionFieldValue"><?php echo StringUtil::encodeHTML(\get_class($e)); ?></p>
				</li>
				<li>
					<p class="exceptionFieldTitle">Error Message<span
							class="exceptionColon">:</span></p>
				...
				<li>
					...
					<p class="exceptionFieldValue"
					   style="word-break: break-all"><?php echo StringUtil::encodeHTML(sanitizePath($e->getFile())); ?>
    ...
    return \array_map(static function ($item) use ($ignorePaths) {
        ...
        if (!$ignorePaths) {
            $item['args'] = \array_map(static function ($item) {
                ...
                if (\preg_match('~^(' . \preg_quote($_SERVER['DOCUMENT_ROOT'], '~') . '|' . \preg_quote(WCF_DIR,
                        '~') . ')~', $item)) {
