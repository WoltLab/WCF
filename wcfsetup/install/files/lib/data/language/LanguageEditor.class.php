...
class LanguageEditor extends DatabaseObjectEditor implements IEditableCachedObject
{
    ...
    public function delete()
    {
        ...
        self::deleteLanguageFiles((string)$this->languageID);
    }
    ...
    protected function writeLanguageFiles(array $languageCategoryIDs)
    {
        ...
        foreach ($items as $languageCategoryID => $languageItems) {
            ...
            // `*/` must be stripped, otherwise a crafted language code could
            // terminate the comment early and have the remainder evaluated as
            // PHP. We're replacing `*` and `/` unconditionally because they
            // could be abused in `**//` and similar.
            $writer->write("<?php\n/**\n* WoltLab Suite\n* language: " . \str_replace(['*', '/'], '', $this->languageCode) . "\n* encoding: UTF-8\n* category: " . $category->languageCategory . "\n* generated at " . \gmdate("r") . "\n* \n* DO NOT EDIT THIS FILE\n*/\n");
            foreach ($languageItems as $languageItem => $languageItemValue) {
                // The array keys must be exported rather than interpolated, otherwise a phrase
                // name containing a single quote would inject arbitrary PHP into this file.
                $writer->write("\$this->items[" . \var_export((string)$languageItem, true) . "] = " . \var_export($languageItemValue, true) . ";\n");
    ...
    public function updateFromXML(XML $xml, $packageID, $updateFiles = true, $updateExistingItems = true)
    {
        ...
        foreach ($categories as $category) {
            ...
            self::validateCategoryName($categoryName);

            $usedCategories[$categoryName] = 0;
        }
    ...
    final public static function validateItemName(string $itemName, string $categoryName): void
    {
        ...
        if (!\preg_match(Language::PHRASE_PATTERN, $itemName)) {
            throw new \InvalidArgumentException("The name '{$itemName}' is not a valid phrase name.");
        }
    ...
    /**
     * Verifies that the given phrase category name is valid.
     * Throws an exception otherwise.
     *
     * @since 6.2
     */
    final public static function validateCategoryName(string $categoryName): void
    {
        if (empty($categoryName)) {
            throw new \InvalidArgumentException("The name attribute is missing or empty.");
        }

        if (!\preg_match(Language::CATEGORY_PATTERN, $categoryName)) {
            throw new \InvalidArgumentException("The name '{$categoryName}' is not a valid phrase category name.");
        }
    }
    ...
    public function updateItems(
        ...
        // update the relevant language files
        self::deleteLanguageFiles((string)$this->languageID, $category->languageCategory);
