...
class Route {
	 ...
	 */
	public function __construct($routeName, $routeSchema, $controller = null, $isACPRoute = false, $partsPattern = null) {
	...
	protected function validateSchema() {
		...
		foreach ($this->routeSchemaParts as &$part) {
			if (!Regex::compile('^{[a-zA-Z]+}$')->match($part)) {
	...
	public function canHandle(array $components) {
		foreach ($this->components as $component) {
			if (isset($components[$component->name])) {
				if (!$component->matches($components[$component->name])) {
					return false;
				}
			}
			else {
