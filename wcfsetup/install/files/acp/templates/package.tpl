		...
		<fieldset>
			...
			{if $package->license}
				<dl>
					...
					<dd>{if $package->licenseURL}<a href="{@$__wcf->getPath()}acp/dereferrer.php?url={$package->licenseURL|rawurlencode}" class="externalURL">{$package->license}</a>{else}{$package->license}{/if}</dd>
				</dl>
			{/if}
