		...
		<table>
			<thead>
				<tr class="tableHead">
					...
					<th class="columnGroupName"><p class="emptyHead">{lang}wcf.acp.group.groupName{/lang}</p></th>
			...
			<tbody>
				{foreach from=$groups item=group}
					<tr id="groupContainer{@$group->groupID}">
						...
						<td class="columnGroupIdentifier columnText">{$group->groupIdentifier}</td>
						<td class="columnGroupName columnText">{if $group->isEditable()}<p><a title="{lang}wcf.acp.group.edit{/lang}" href="index.php?form=UserGroupEdit&amp;groupID={@$group->groupID}{@SID_ARG_2ND}">{lang}{$group->groupIdentifier}{/lang}</a>{else}{lang}{$group->groupIdentifier}{/lang}</p>{/if}</td>
