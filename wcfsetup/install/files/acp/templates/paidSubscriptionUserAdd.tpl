...
<header class="contentHeader">
	...
	<nav class="contentHeaderNavigation">
		<ul>
			{if $action == 'add'}
				<li><a href="{link controller='PaidSubscriptionList'}{/link}" class="button">{icon size=16 name='list' type='solid'} <span>{lang}wcf.acp.menu.link.paidSubscription.list{/lang}</span></a></li>
			{else}
				<li><a href="{link controller='PaidSubscriptionUserList'}{/link}" class="button">{icon size=16 name='list' type='solid'} <span>{lang}wcf.acp.menu.link.paidSubscription.user.list{/lang}</span></a></li>
			{/if}
