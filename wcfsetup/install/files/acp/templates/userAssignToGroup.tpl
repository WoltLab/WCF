...
<form method="post" action="{link controller='UserAssignToGroup'}{/link}">
	...
	<div class="wcf-box wcf-marginTop wcf-boxPadding wcf-shadow1">
		...
		<fieldset>
			...
			<ul>
				{implode from=$users item=$user}<li><a href="{link controller='UserEdit' controllerObject=$user}{/link}" class="wcf-badge wcf-badgeButton wcf-button">{$user}</a></li>{/implode}
			</ul>
		</fieldset>	
