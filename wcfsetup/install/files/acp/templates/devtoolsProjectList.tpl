...
<header class="contentHeader">
	...
	<nav class="contentHeaderNavigation">
		<ul>
			<li><a href="#" class="button jsDevtoolsProjectQuickSetupButton">{icon size=16 name='search' type='solid'} <span>{lang}wcf.acp.devtools.project.quickSetup{/lang}</span></a></li>
			<li><a href="{link controller='DevtoolsProjectAdd'}{/link}" class="button">{icon size=16 name='plus' type='solid'} <span>{lang}wcf.acp.devtools.project.add{/lang}</span></a></li>
				...
				{content}
					{foreach from=$objects item=object}
						<tr class="jsObjectRow jsObjectActionObject" data-object-id="{@$object->getObjectID()}">
							...
							<td class="columnIcon">
								<a href="{link controller='DevtoolsProjectEdit' id=$object->getObjectID()}{/link}" title="{lang}wcf.global.button.edit{/lang}" class="jsTooltip">{icon size=16 name='pencil' type='solid'}</a>
