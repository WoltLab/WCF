	...
	{hascontent}
		<nav class="contentHeaderNavigation">
			<ul>
				{content}
					{if $cacheData.files}
						<li><a onclick="WCF.System.Confirmation.show('{jslang}wcf.acp.cache.clear.sure{/jslang}', $.proxy(function (action) { if (action == 'confirm') window.location.href = $(this).attr('href'); }, this)); return false;" href="{link controller='CacheClear'}{/link}" class="button">{icon size=16 name='xmark'} <span>{lang}wcf.acp.cache.button.clear{/lang}</span></a></li>
					{/if}
	...
	{foreach from=$cacheTypeCaches key='cache' item='files'}
		...
		{if $files|count}
			<section class="section tabularBox tabularBoxTitle">
				<header>
					<h2>
						<a class="jsCollapsible jsTooltip" data-is-open="0" data-collapsible-container="cache{@$cacheIndex}" title="{lang}wcf.global.button.collapsible{/lang}">{icon size=16 name='chevron-right'}</a>
