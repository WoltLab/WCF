...
<header class="contentHeader">
	...
	<nav class="contentHeaderNavigation">
		<ul>
			<li><button class="button" id="sitemapRebuildButton">{icon size=16 name='arrows-rotate' type='solid'} <span>{lang}wcf.acp.rebuildData.com.woltlab.wcf.sitemap{/lang}</span></button></li>
				...
				{foreach from=$sitemapObjectTypes item=object}
					<tr class="sitemapObjectRow jsObjectActionObject" data-object-id="{@$object->getObjectID()}">
						<td class="columnIcon">
							...
							<a href="{link controller="SitemapEdit"}objectType={$object->objectType}{/link}" title="{lang}wcf.global.button.edit{/lang}" class="jsTooltip">{icon size=16 name='pencil' type='solid'}</a>
						</td>
