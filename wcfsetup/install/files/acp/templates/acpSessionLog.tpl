		...
		<table class="table">
			<thead>
				<tr>
					...
					<th class="columnDate{if $sortField == 'logTime'} active{/if}"><a href="{link controller='ACPSessionLog' id=$sessionLogID}pageNo={@$pageNo}&sortField=logTime&sortOrder={if $sortField == 'logTime' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.acp.sessionLog.time{/lang}{if $sortField == 'logTime'} <img src="{@$__wcf->getPath()}icon/sort{@$sortOrder}.svg" alt="" />{/if}</a></th>
				...
				{content}
					{foreach from=$objects item=sessionAccessLog}
						<tr>
							...
							<td class="columnDate columnTime"><p>{@$sessionAccessLog->logTime|time}</p></td>
