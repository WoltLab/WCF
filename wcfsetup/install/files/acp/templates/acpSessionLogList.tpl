		...
		<table class="table">
			<thead>
				<tr>
					...
					<th class="columnDate columnTime{if $sortField == 'logTime'} active{/if}"><a href="{link controller='ACPSessionLogList'}pageNo={@$pageNo}&sortField=logTime&sortOrder={if $sortField == 'logTime' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.acp.sessionLog.time{/lang}{if $sortField == 'logTime'} <img src="{@$__wcf->getPath()}icon/sort{@$sortOrder}.svg" alt="" />{/if}</a></th>
				...
				{content}
					{foreach from=$objects item=sessionLog}
						<tr class="{if $sessionLog->active} activeContainer{/if}">
							...
							<td class="columnDate columnTime"><p>{@$sessionLog->logTime|time}</p></td>
