{foreach from=$plugins key=packageID item=package}
	<li class="wcf-infoPackagePlugin wcf-box wcf-button jsPackageContainer">
		<div>
			<a href="{link controller='Package' id=$packageID}{/link}" title="{$package->getName()}" style="background-image: url('{@$__wcf->getPath()}icon/plugin.svg');">
		...
		<footer>
			<nav>
				<ul class="wcf-smallButtonBar">
					<li><a href="{link controller='Package' id=$packageID}{/link}" title="{$package->getName()}" class="jsTooltip"><img src="{@$__wcf->getPath()}icon/info1.svg" alt="" /> <span>{lang}wcf.acp.package.button.info{/lang}</span></a></li>
