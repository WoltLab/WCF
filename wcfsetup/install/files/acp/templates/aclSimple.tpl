...
{if !$__aclSimpleNamePrefix|isset}{assign var='__aclSimpleNamePrefix' value=''}{/if}
	...
	<dl>
		...
		<dd>
			<ol class="flexibleButtonGroup">
				<li>
					<input type="radio" id="{@$__aclSimplePrefix}aclAllowAll" name="{@$__aclSimpleNamePrefix}aclValues[allowAll]" value="1"{if $aclValues[allowAll]} checked{/if}>
				...
				<li>
					<input type="radio" id="{@$__aclSimplePrefix}aclAllowAll_no" name="{@$__aclSimpleNamePrefix}aclValues[allowAll]" value="0"{if !$aclValues[allowAll]} checked{/if}>
			...
			<ul id="{@$__aclSimplePrefix}aclAccessList" class="aclList containerList">
				{foreach from=$aclValues[group] item=aclGroup}
					<li>
						...
						<input type="hidden" name="{@$__aclSimpleNamePrefix}aclValues[group][]" value="{@$aclGroup->groupID}">
					</li>
				{/foreach}
				{foreach from=$aclValues[user] item=aclUser}
					<li>
						...
						<span class="icon icon16 fa-times pointer jsTooltip" title="{lang}wcf.global.button.delete{/lang}"></span>
						<input type="hidden" name="{@$__aclSimpleNamePrefix}aclValues[user][]" value="{@$aclUser->userID}">
					</li>
				{/foreach}
			</ul>
		</dd>
...
<script data-relocate="true">
	require(['WoltLabSuite/Core/Ui/Acl/Simple'], function(UiAclSimple) {
		new UiAclSimple('{@$__aclSimplePrefix}', '{@$__aclSimpleNamePrefix}');
	});
</script>
