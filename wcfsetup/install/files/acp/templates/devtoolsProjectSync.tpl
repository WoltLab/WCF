...
<header class="contentHeader">
	...
	<nav class="contentHeaderNavigation">
		<ul>
			{if $object->validate() === ''}
				<li><a href="{link controller='DevtoolsProjectPipList' id=$object->getObjectID()}{/link}" class="button">{icon size=16 name='list' type='solid'} <span>{lang}wcf.acp.devtools.project.pips{/lang}</span></a></li>
			{/if}
			<li><a href="{link controller='DevtoolsProjectEdit' id=$object->getObjectID()}{/link}" class="button">{icon size=16 name='pencil' type='solid'} <span>{lang}wcf.acp.devtools.project.edit{/lang}</span></a></li>
