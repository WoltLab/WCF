...
<header class="contentHeader">
	...
	<nav class="contentHeaderNavigation">
		<ul>
			<li><a href="{link controller='TagAdd'}{/link}" class="button">{icon size=16 name='plus'} <span>{lang}wcf.acp.tag.add{/lang}</span></a></li>
			...
			<tbody class="jsReloadPageWhenEmpty">
				{foreach from=$objects item=tag}
					<tr class="jsTagRow jsClipboardObject jsObjectActionObject" data-object-id="{@$tag->tagID}">
						...
						<td class="columnIcon">
							<a href="{link controller='TagEdit' object=$tag}{/link}" title="{lang}wcf.global.button.edit{/lang}" class="jsTooltip">{icon size=16 name='pencil'}</a>
