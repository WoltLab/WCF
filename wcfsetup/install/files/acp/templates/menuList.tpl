...
<header class="contentHeader">
	...
	<nav class="contentHeaderNavigation">
		<ul>
			<li><a href="{link controller='MenuAdd'}{/link}" class="button">{icon size=16 name='plus'} <span>{lang}wcf.acp.menu.add{/lang}</span></a></li>
			...
			<tbody class="jsReloadPageWhenEmpty">
				{foreach from=$objects item=menu}
					<tr class="jsMenuRow jsObjectActionObject" data-object-id="{@$menu->getObjectID()}">
						<td class="columnIcon">
							<a href="{link controller='MenuEdit' id=$menu->menuID}{/link}" title="{lang}wcf.global.button.edit{/lang}" class="jsTooltip">{icon size=16 name='pencil'}</a>
							{if $menu->canDelete()}
