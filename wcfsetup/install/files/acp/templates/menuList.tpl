...
<header class="contentHeader">
	...
	<nav class="contentHeaderNavigation">
		<ul>
			<li><a href="{link controller='MenuAdd'}{/link}" class="button">{icon size=16 name='plus' type='solid'} <span>{lang}wcf.acp.menu.add{/lang}</span></a></li>
			...
			<tbody class="jsReloadPageWhenEmpty">
				{foreach from=$objects item=menu}
					<tr class="jsMenuRow jsObjectActionObject" data-object-id="{@$menu->getObjectID()}">
						<td class="columnIcon">
							<a href="{link controller='MenuEdit' id=$menu->menuID}{/link}" title="{lang}wcf.global.button.edit{/lang}" class="jsTooltip">{icon size=16 name='pencil' type='solid'}</a>
							{if $menu->canDelete()}
								{objectAction action="delete" objectTitle=$menu->getTitle()}
							{else}
								<span class="icon icon16 fa-times disabled" title="{lang}wcf.global.button.delete{/lang}"></span>
							{/if}
							<a href="{link controller='MenuItemList' id=$menu->menuID}{/link}" title="{lang}wcf.acp.menu.item.list{/lang}" class="jsTooltip">{icon size=16 name='list' type='solid'}</a>
							<a href="{link controller='MenuItemAdd'}menuID={@$menu->menuID}{/link}" title="{lang}wcf.acp.menu.item.add{/lang}" class="jsTooltip">{icon size=16 name='plus' type='solid'}</a>
