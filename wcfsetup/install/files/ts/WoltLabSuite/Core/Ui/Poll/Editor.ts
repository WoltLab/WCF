...
class UiPollEditor {
  ...
  constructor(containerId: string, pollOptions: PollOption[], wysiwygId: string, options: UiPollEditorOptions) {
    ...
    this.resultsRequireVoteYesField = document.getElementById(this.wysiwygId + 'Poll_resultsRequireVote') as HTMLInputElement;
    this.resultsRequireVoteNoField = document.getElementById(this.wysiwygId + 'Poll_resultsRequireVote_no') as HTMLInputElement;
    ...
    this.options = Core.extend({
      isAjax: false,
      maxOptions: 20,
    }, options) as UiPollEditorOptions;
