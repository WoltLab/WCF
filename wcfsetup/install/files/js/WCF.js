...
WCF.Collapsible.Remote = Class.extend({
	...
	_createButton: function(containerID, buttonContainer) {
		...
		var $button = $('<a class="wcf-balloonTooltip" title="'+WCF.Language.get('wcf.global.button.' + ($isOpen ? 'close' : 'open'))+'"><img src="' + WCF.Icon.get('wcf.icon.' + ($isOpen ? 'opened' : 'closed')) + '" alt="" /></a>').prependTo(buttonContainer);
...
$.widget('ui.wcfSidebar', {
	...
	_create: function() {
		...
		this._button = $('<span class="wcf-collapsibleSidebarButton" title="' + WCF.Language.get('wcf.global.button.close') + '"><span></span></span>').appendTo(this._container);
