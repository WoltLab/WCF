...
if (COMPILER_TARGET_DEFAULT) {
	...
	WCF.Poll.Management = Class.extend({
		...
		_createOption: function (optionValue, optionID, insertAfter) {
			...
			$('<button type="button" class="jsTooltip jsAddOption" title="' + WCF.Language.get('wcf.poll.button.addOption') + '"><fa-icon size="16" name="plus"></fa-icon></button>').click($.proxy(this._addOption, this)).appendTo($container);
			$('<button type="button" class="jsTooltip jsDeleteOption" title="' + WCF.Language.get('wcf.poll.button.removeOption') + '"><fa-icon size="16" name="xmark"></fa-icon></button>').click($.proxy(this._removeOption, this)).appendTo($container);
