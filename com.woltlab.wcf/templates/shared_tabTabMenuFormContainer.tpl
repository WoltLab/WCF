		...
		<ul>
			{foreach from=$container item='child'}
				{if $child->isAvailable()}
					<li data-name="{$child->getPrefixedId()}Container"{if !$child->checkDependencies()} style="display: none;"{/if}><a{if $container->usesAnchors()} href="#{$child->getPrefixedId()|rawurlencode}Container"{/if}>{unsafe:$child->getLabel()}</a></li>
				{/if}
			{/foreach}
		</ul>
	</nav>
