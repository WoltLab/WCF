				...
				<div class="commentResponse__footer">
					...
					<div class="commentResponse__buttons">
						{if MODULE_LIKE && $commentManager->supportsLike() && $__wcf->session->getPermission('user.like.canLike') && $response->userID != $__wcf->user->userID}
							<button
								type="button"
								class="commentResponse__button commentResponse__button--react reactButton jsTooltip {if $likeData[response][$response->responseID]|isset && $likeData[response][$response->responseID]->reactionTypeID} active{/if}"
