<div class="containerHeadline">
	<h3><a href="{link controller='User' object=$user}{/link}" class="username userOnlineMark" data-user-id="{$user->userID}">{$user->username}</a>{if $user->banned} <span class="icon icon16 fa-lock jsTooltip jsUserBanned" title="{lang}wcf.user.banned{/lang}"></span>{/if}
		{if MODULE_USER_RANK}
