	...
	<dl>
		...
		<dd>
			<ol role="group" aria-label="{lang}wcf.acl.allowAll{/lang}" class="flexibleButtonGroup">
				<li>
					<input type="radio" id="{$__aclSimplePrefix}aclAllowAll" name="{unsafe:$__aclInputName}[allowAll]" value="1"{if $aclValues[allowAll]} checked{/if}>
					<label for="{$__aclSimplePrefix}aclAllowAll" class="green">{icon name='check'} {lang}wcf.acp.option.type.boolean.yes{/lang}</label>
				</li>
				<li>
					<input type="radio" id="{$__aclSimplePrefix}aclAllowAll_no" name="{unsafe:$__aclInputName}[allowAll]" value="0"{if !$aclValues[allowAll]} checked{/if}>
			...
			<ul id="{$__aclSimplePrefix}aclAccessList" class="aclList">
				{foreach from=$aclValues[group] item=aclGroup}
					<li class="aclListItem">
						...
						<input type="hidden" name="{unsafe:$__aclInputName}[group][]" value="{$aclGroup->groupID}">
					</li>
				{/foreach}
				{foreach from=$aclValues[user] item=aclUser}
					<li class="aclListItem">
						...
						<input type="hidden" name="{unsafe:$__aclInputName}[user][]" value="{$aclUser->userID}">
					</li>
				{/foreach}
			</ul>
		</dd>
