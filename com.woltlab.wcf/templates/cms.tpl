...
{capture assign='contentInteractionButtons'}
	{if $page->showShareButtons()}
		<a href="{$content->getLink()}" class="contentInteractionButton button small wsShareButton jsOnly" data-link-title="{$content->getTitle()}">{icon size=16 name='share'} <span>{lang}wcf.message.share{/lang}</span></a>
	{/if}
	...
	{if $__wcf->getSession()->getPermission('admin.content.cms.canManagePage')}<a href="{link controller='PageEdit' id=$page->pageID isACP=true}{/link}" class="contentInteractionButton button small">{icon size=16 name='pencil'} <span>{lang}wcf.acp.page.edit{/lang}</span></a>{/if}
{/capture}
