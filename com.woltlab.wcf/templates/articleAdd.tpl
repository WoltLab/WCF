		...
		{hascontent}
			<nav class="contentHeaderNavigation">
				<ul>
					{content}
						{if $action == 'edit'}
							<li><a href="{$article->getLink()}" class="button buttonPrimary">{icon size=16 name='magnifying-glass'} <span>{lang}wcf.acp.article.button.viewArticle{/lang}</span></a></li>
						{/if}
...
{capture assign='contentInteractionButtons'}
	{if $action == 'edit'}
		{if $article->canDelete()}
			<button class="contentInteractionButton button small jsButtonRestore" data-confirm-message-html="{lang __encode=true isArticleEdit=true}wcf.acp.article.restore.confirmMessage{/lang}"{if !$article->isDeleted} style="display: none"{/if}>{icon size=16 name='rotate-left'} <span>{lang}wcf.global.button.restore{/lang}</span></button>
			<button class="contentInteractionButton button small jsButtonDelete" data-confirm-message-html="{lang __encode=true isArticleEdit=true}wcf.acp.article.delete.confirmMessage{/lang}"{if !$article->isDeleted} style="display: none"{/if}>{icon size=16 name='xmark'} <span>{lang}wcf.global.button.delete{/lang}</span></button>
			<button class="contentInteractionButton button small jsButtonTrash" data-confirm-message-html="{lang __encode=true isArticleEdit=true}wcf.acp.article.trash.confirmMessage{/lang}"{if $article->isDeleted} style="display: none"{/if}>{icon size=16 name='xmark'} <span>{lang}wcf.global.button.trash{/lang}</span></button>
		{/if}
		{if $languages|count > 1 || $article->isMultilingual}
			<button class="contentInteractionButton button small jsButtonToggleI18n">{icon size=16 name='language'} <span>{lang}wcf.acp.article.button.toggleI18n{/lang}</span></button>
		{/if}
	{/if}
{/capture}
