		...
		<div class="formSubmit jsOnly"{if !$poll->canVote() && $__pollView === 'result' && !$poll->canSeeResult()} style="display: none"{/if}>
			{content}
				...
				{if $poll->canViewParticipants() || ($poll->canVote() && $poll->isPublic)}
					<button type="button" class="button small showPollParticipantsButton"{if $__pollView === 'vote' || !$poll->canSeeResult()} hidden{/if}>{lang}wcf.poll.button.showParticipants{/lang}</button>
				{/if}
